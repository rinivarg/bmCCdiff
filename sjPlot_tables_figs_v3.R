...
library("table1"); library("robustlmm")
...
fitted = visreg(m12, xvar = "FA_val", by = "CC_region",scale = "response",
                strip.names = FALSE,
...
fitted + scale_color_manual(values =new_cols, name = "Region",
                            ...
                            coord_cartesian(xlim = c(0.45,0.9),,ylim=c(1.5,3.8),expand = 0) + 
                            scale_y_continuous(breaks = round(seq(1.5, 3.8, by = 0.5),1)) +
